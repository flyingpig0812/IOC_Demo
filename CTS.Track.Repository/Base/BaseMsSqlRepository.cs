...
namespace CTS.Track.Repository.Base
{
    public class BaseMsSqlRepository<TEntity> : DbMsSqlContext<TEntity>, IBaseRepository<TEntity> where TEntity : class, new()
    {
        ...
        public async Task<bool> Add(List<TEntity> models)
        {
            return await Task.Run(() =>
            {
                //var result = CurrentDb.AsInsertable(models).IgnoreColumns(true).ExecuteCommand();
                var result = CurrentDb.AsInsertable(models).ExecuteCommand();
                return result > 0;
            });
        }
